...
for k = 1:length(t)
    if k-1 > 0
        theta2(k+1) = 2.*theta2(k) - theta2(k-1) + dw2_dt(theta1(k), theta2(k), gl1, gl2, km2, l1l2).*(h^2);
        theta1(k+1) = 2.*theta1(k) - theta1(k-1) + dw1_dt(theta1(k), theta2(k), gl1, gl2, km1, l2l1).*(h^2);
        
        %omegas are not used in calculation
        omega2(k) = (theta2(k+1) - theta2(k-1))/(2*h);
        omega1(k) = (theta1(k+1) - theta1(k-1))/(2*h);
    else
        omega2(k+1) = omega2(k) + dw2_dt(theta1(k), theta2(k), gl1, gl2, km2, l1l2).*h;
        omega1(k+1) = omega1(k) + dw1_dt(theta1(k), theta2(k), gl1, gl2, km1, l2l1).*h;
        theta2(k+1) = theta2(k) + omega2(k).*h;
        theta1(k+1) = theta1(k) + omega1(k).*h;
    end
end
