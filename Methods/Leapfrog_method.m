...
for k = 1:(length(t)-1)
    if k-1 > 0
        omega2(k+1) = omega2(k-1) + dw2_dt(theta1(k), theta2(k), gl1, gl2, km2, l1l2).*h*2;
        omega1(k+1) = omega1(k-1) + dw1_dt(theta1(k), theta2(k), gl1, gl2, km1, l2l1).*h*2;
        theta2(k+2) = theta2(k) + 2*h*omega2(k+1);
        theta1(k+2) = theta1(k) + 2*h*omega1(k+1);
    else
